...
def optimize_params_fine_tuned(x, lambda_n, N, initial_theta):
    def objective(params):
        alpha, k = params[:2]  # alpha 和 k 是优化的两个参数
        theta = params[2:]     # 剩余的参数是 theta_n
        ...
        # 添加惩罚项，使得 k 越接近 1 越好
        k_penalty = (k - 1) ** 2  # 惩罚项，k 偏离 1 时增大

        # 返回目标函数值，目标是最小化 delta 和惩罚项
        return float(abs(delta_value) + 1e6 * k_penalty)  # 1e6 是惩罚项的权重
